/**
 * Original source: https://github.com/ClickHouse/clickhouse-js/blob/main/src/data_formatter/formatter.ts
 */
...
const supportedJSONFormats = [...singleDocumentJSONFormats, ...streamableJSONFormats] as const;
...
type SingleDocumentStreamableJsonDataFormat = (typeof singleDocumentJSONFormats)[number];
type StreamableJsonDataFormat = (typeof streamableJSONFormats)[number];

// TODO add others formats
const streamableFormat = [...streamableJSONFormats, ...supportedRawFormats] as const;
type StreamableDataFormat = (typeof streamableFormat)[number];

function isNotStreamableJSONFamily(format: DataFormat): format is SingleDocumentStreamableJsonDataFormat {
  // @ts-expect-error JSON is not assignable to notStreamableJSONFormats
  return singleDocumentJSONFormats.includes(format);
}

export function isStreamableJSONFamily(format: DataFormat): format is StreamableJsonDataFormat {
