/**
 * Original source: https://github.com/ClickHouse/clickhouse-js/blob/main/src/data_formatter/format_query_params.ts
 */
...
export function formatQueryParams(value: any, wrapStringInQuotes = false): string {
  ...
  if (value instanceof Date) {
    ...
    const time = `${withPadding(value.getHours())}:${withPadding(value.getMinutes())}:${withPadding(
      value.getSeconds(),
    )}`;
...
export function replaceAll(input: string, replace_char: string, new_char: string): string {
